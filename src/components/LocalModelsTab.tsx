        ...
        <div className="settings-group" style={{ marginBottom: 16 }}>
          ...
          <div style={{ padding: "12px 16px" }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button className="btn btn-sm" onClick={async () => { try { const dir = await open({ directory: true, multiple: false }); if (dir) { setLocalModelId(dir); startLocalLLM(dir); } } catch { /* dialog cancelled */ } }} title={t("local.select_dir")} style={{ minWidth: 100, padding: "8px 16px" }}>📂 {t("local.select_dir")}</button>

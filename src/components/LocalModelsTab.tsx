...
function ModelDetailPanel({ modelId, detailData, detailLoading, downloadProgress, downloadModel, pauseDownload, cancelDownload, resumeDownload, startLocalLLM, deleteModelFile, fetchModelDetail, onClose }: {
  ...
  if (!modelId) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "#d1d5db", fontSize: 13, padding: 40, textAlign: "center" }}>
      <div><div style={{ fontSize: 40, marginBottom: 12 }}>📦</div>从左侧列表选择一个模型<div style={{ fontSize: 10, marginTop: 4, color: "#e5e7eb" }}>查看详情、可用版本和 README</div></div>
    </div>
  );
  if (detailLoading) return (
    <div style={{ textAlign: "center", padding: 60 }}>
      ...
      <span style={{ color: "#9ca3af", fontSize: 12 }}>加载中...</span>
    </div>
  );
  if (!detailData || detailData.status === "error") return (
    <div style={{ textAlign: "center", padding: 40, color: "#ef4444", fontSize: 12 }}>
      加载失败 — <button className="btn btn-sm btn-ghost" onClick={() => fetchModelDetail(modelId)}>重试</button>
    </div>
  );
...
export default function LocalModelsTab(props: Props) {
  ...
  const deleteModelFile = async (modelId: string) => { try { const resp = await fetch(SIDECAR + "/v1/local-llm/delete-model", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model_id: modelId }) }); const data = await resp.json(); showToast(data.message || "已删除"); if (data.status === "ok" && detailModelId) fetchModelDetail(detailModelId); } catch(e) { console.error(e); showToast("删除失败"); } };
    ...
    <div style={{ position: "relative" }}>
        ...
        <div style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {isRunning && <>
              ...
              <div style={{ marginTop: 8, display: "flex", gap: 8 }}><button className="btn btn-sm btn-primary" onClick={stopLocalLLM} style={{ padding: "6px 16px" }}>⏹ {t("local.stop_btn")}</button><button className="btn btn-sm btn-ghost" style={{ padding: "6px 16px", color: "var(--danger)" }} onClick={async () => { const mid = localLLMStatus.model_id; if (!mid) return; const ok = await ask(`确定要删除该模型吗？\n${mid}`); if (!ok) return; try { await stopLocalLLM(); const resp = await fetch(SIDECAR + "/v1/local-llm/delete-model", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model_id: mid }) }); const data = await resp.json(); showToast(data.message || "已删除"); } catch(e) { console.error(e); showToast("删除失败"); } }}>🗑 删除模型</button></div>
            </>}
          </div>
        </div>
      </div>
      ...
      <div className="settings-group" style={{ marginBottom: 16 }}>
        ...
        <div style={{ padding: "12px 16px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8, fontSize: 10, color: "var(--text-muted)", alignItems: "center" }}>
            <button className="btn btn-sm btn-ghost" style={{ fontSize: 10, padding: "2px 8px" }} onClick={() => fetchContextEstimate(localModelId || undefined)}>🔄 重新检测</button>
            <button className="btn btn-sm btn-ghost" style={{ fontSize: 10, padding: "2px 8px" }} onClick={() => startLocalLLM()} disabled={!localLLMStatus.model_id}>🔄 重新加载模型</button>
            {contextEstimate && <><span>| 最大安全: {contextEstimate.max_context.toLocaleString()} tokens</span><span>| 重启模型后生效</span></>}
          </div>
          {contextEstimate && contextLimit > contextEstimate.max_context && <div style={{ marginTop: 8, fontSize: 10, color: "var(--danger)" }}>⚠️ 当前设置超过安全上限（{contextEstimate.max_context.toLocaleString()}），可能导致内存不足！</div>}
        </div>
      </div>
      ...
      <div className="settings-group" style={{ marginBottom: 16 }}>
        ...
        <div style={{ padding: "12px 16px" }}>
          ...
          <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 6 }}>浏览和搜索 HuggingFace 上的 GGUF/MLX 模型</div>
        </div>
      </div>
          ...
          <div style={{ position: "absolute", top: 16, left: 16, right: 16, bottom: 16, background: "var(--bg-card)", borderRadius: 14, boxShadow: "0 20px 60px rgba(0,0,0,0.25)", zIndex: 200, display: "flex", overflow: "hidden" }}>
            <div style={{ width: 340, minWidth: 280, flexShrink: 0, borderRight: "1px solid var(--border-default)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div className="custom-scrollbar" style={{ flex: 1, overflowY: "auto", padding: "8px 12px" }}>
                {searching && <div style={{ textAlign: "center", padding: 40, color: "#9ca3af", fontSize: 12 }}>搜索中...</div>}
                {!searching && filteredResults.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "#9ca3af", fontSize: 12 }}>{hfSearch ? "无结果" : "热门 GGUF 模型 · 输入关键词搜索更多"}</div>}

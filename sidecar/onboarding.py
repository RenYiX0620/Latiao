...
_INTRO = {
    ...
    "ja": ("【初回ガイド】ユーザーとの初めての会話です。次の形式を厳守してください（雑談・箇条書き・ツール呼び出しは不要）："
           ...
           "**最後に必ずこの質問で締めくくる**：{q}"),
    "ru": ("【Первое знакомство】Это первый разговор пользователя с вами. Строго следуйте формату: без светской беседы, "
           "без списков, без вызова инструментов. Сначала представьтесь одним-двумя предложениями (имя, работа локально "
           "на этом Mac, умение читать/писать файлы, выполнять команды, искать в интернете), затем **последним "
           "предложением задайте ровно этот вопрос**: {q}"),
}
...
_CONFIRM_NEXT = {
    ...
    "ru": "Система записала {label} как «{value}».",}
...
_ASK_NEXT = {
    ...
    "ru": ("Подтвердите одним предложением, затем **завершите ответ ровно этим вопросом**: {q} — без повторного самопредставления, без списков, без вызова инструментов."),}
...
_CONFIRM_DONE = {
    ...
    "ru": "Система записала {label} как «{value}»; знакомство завершено.",}
...
_RETRY = {
    ...
    "ru": ("Пользователь ещё не дал чёткого ответа. Сначала ответьте на то, что он спросил, затем естественно задайте вопрос снова: {q} — не делайте вид, что что-то записано."),}
...
_DONE_NOTE = {
    ...
    "ru": "Знакомство завершено — больше не спрашивайте. Позже пользователь может сказать «зови меня X» или запустить знакомство заново в настройках.",}
...
_SKIP_ALL_NOTE = {
    ...
    "ru": "Пользователь отказался от настройки. Кратко ответьте и продолжайте как обычно — больше не спрашивайте.",}

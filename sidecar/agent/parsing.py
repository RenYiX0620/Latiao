...
# Qwen2.5/Qwen3 另一支 XML 方言：<tool_call><function=NAME><parameter=K>V</parameter></function></tool_call>
# 09-19 实测（Qwen3.8-27B-UD-Q6_K_M GGUF）：此前四层解析器都不认 → 整段 XML 被当回答
# 交付给用户，还因拉丁字符占多触发语言闸门的误判提示，工具一次都没执行。
_TOOLCALL_FN_XML_RE = re.compile(
    r"<tool_call>\s*<function=([^>\s]+)\s*>(.*?)</function>\s*</tool_call>", re.DOTALL)
_TOOLCALL_PARAM_RE = re.compile(r"<parameter=([^>\s]+)\s*>(.*?)</parameter>", re.DOTALL)
# 任何未被识别的 <tool_call> 块（兜底：不把标记原样交付用户）
_TOOLCALL_ANY_RE = re.compile(r"<tool_call>.*?</tool_call>", re.DOTALL)
...
# 参数名别名（09-19 实测）：模型常给通用键名（name/file/command），而工具 schema 用
# 专用键（skill_name/path/cmd）。键名不匹配会让工具报"缺少参数"、白跑一轮。
_ARG_ALIASES = {
    "use_skill": {"name": "skill_name", "skill": "skill_name", "skill_id": "skill_name"},
    "read_file": {"file": "path", "filename": "path", "file_path": "path"},
    "write_file": {"file": "path", "filename": "path", "file_path": "path", "content_text": "content"},
    "run_cmd": {"command": "cmd", "shell": "cmd"},
    "tavily_search": {"q": "query", "search_query": "query", "keywords": "query"},
    "mx_query": {"q": "query", "keywords": "query", "symbol": "query"},
    "search_files": {"query": "pattern", "name": "pattern"},
    "list_dir": {"dir": "path", "directory": "path"},
}


def _apply_arg_aliases(tool_name: str, args: dict) -> dict:
    """把模型给的通用键名映射到工具 schema 的键名（缺专有键时才映射）。"""
    aliases = _ARG_ALIASES.get(tool_name or "")
    if not aliases or not isinstance(args, dict):
        return args
    out = dict(args)
    for generic, canonical in aliases.items():
        if generic in out and canonical not in out:
            out[canonical] = out.pop(generic)
    return out


# ── 通用工具调用回收（09-19）：这个模型会随采样输出各种"方言"：<tool_call>+<function=>
# <function_calls><invoke name="x">、甚至漏掉左尖括号的 `parameter name="q">`。逐方言打
# 补丁打不完，这里做通用回收：识别任何 invoke/function 标记 + 已知工具名 + 参数键值。
_FUNCTION_CALLS_RE = re.compile(r"<function_calls>.*?</function_calls>", re.DOTALL)
_INVOKE_RE = re.compile(r"<invoke\s+name=[\"']?([\w.]+)[\"']?\s*>(.*?)</invoke>", re.DOTALL)
_INVOKE_PARAM_RE = re.compile(
    r"<?parameter\s+name=[\"']?([\w.]+)[\"']?\s*>(.*?)(?:</parameter>|(?=<)|$)", re.DOTALL)
# 任意形状的调用标记（清洗用）：<function_calls>/<invoke>/<tool_call>/裸 parameter 片段
...
def _known_tool_names() -> set:
    ...
    try:
        ...
        return {((t.get("function") or {}).get("name") or "") for t in TOOLS} - {""}
    except Exception:
        return {"mx_query", "ak_finance", "tavily_search", "web_search", "bing_search",
                "read_file", "write_file", "list_dir", "search_files", "run_cmd",
                "use_skill", "delegate_task", "open_app", "open_folder",
                "create_cron", "create_skill", "headless_read"}


def _salvage_invoke_style(text: str) -> list:
    """通用回收：<function_calls>/<invoke name="tool">…parameter name="k">v… 形态。"""
    calls = []
    known = _known_tool_names()
    for im in _INVOKE_RE.finditer(_FUNCTION_CALLS_RE.sub(lambda m: m.group(0), text)):
        name = im.group(1)
        if name not in known:
            continue
        args = {}
        for pm in _INVOKE_PARAM_RE.finditer(im.group(2)):
            key = pm.group(1)
        ...
        if not args:
            continue
        calls.append({
            "id": f"local_invoke_{name}_{len(calls)}",
            "type": "function",
            "function": {"name": name,
                         "arguments": json.dumps(_apply_arg_aliases(name, args), ensure_ascii=False)},
        })
    return calls
...
def _parse_prompt_tool_calls(text: str) -> tuple[str, list[dict]]:
    ...
    # ⚠️ 但 09-19 实测（Qwen3.8-27B-UD GGUF）该模型**把工具调用写在 think 块里**：
    # 先剥后解析就永远找不到调用 → 工具不执行、循环空转到步数上限。故若无命中，
    # 回退到含 think 的原文再解析一次（宁可认一个"想了一半"的调用，也别空转）。
    ...
    # Priority 1.25: <tool_call>{"name": …, "arguments": {…}}</tool_call>
    # —— 模型自带的原生 JSON 形态（09-20 实测 Qwen3.8-27B-NEO-CODER 系）。
    # 此前不认这种方言：标记被流式清洗当成工具标记**丢掉**，只剩前后那句
    # "我来帮你分析…先调取数据"，循环于是把"预告"当终答交付 → 用户看到
    # "执行到一半就停"。参数名做容错（name/tool/function、arguments/parameters/args），
    # arguments 允许是 JSON 字符串（有些模板会二次转义）。
    if not tool_calls:
        for _w_idx, _wm in enumerate(_TOOLCALL_ANY_RE.finditer(search_text)):
            _raw = _wm.group(0)
            _body = _raw[_raw.find(">") + 1:_raw.rfind("</tool_call>")].strip()
            _obj = None
            try:
            ...
            _wname = str(_obj.get("name") or _obj.get("tool")
                         or _obj.get("function") or "").strip()
            if not _wname:
                continue
            _wargs = _obj.get("arguments", _obj.get("parameters", _obj.get("args", {})))
            if isinstance(_wargs, str):
                try:
                    _wargs = json.loads(_wargs)
                except (json.JSONDecodeError, ValueError):
                    _wargs = _salvage_tool_args(_wargs)
            if not isinstance(_wargs, dict):
                _wargs = {}
            tool_calls.append({
                "id": f"local_tcw_{_wname}_{_w_idx}",
                "type": "function",
                "function": {"name": _wname,
                             "arguments": json.dumps(_wargs, ensure_ascii=False)},
            })
            used_ranges.append((_wm.start(), _wm.end()))

    # Priority 1.3: Qwen「function=/parameter=」XML 方言（09-19 实测）
    if not tool_calls:
        for f_idx, fm in enumerate(_TOOLCALL_FN_XML_RE.finditer(search_text)):
            _fname = fm.group(1)
            _fargs = {}
            for pm in _TOOLCALL_PARAM_RE.finditer(fm.group(2)):
                _raw = pm.group(2).strip()
                try:
                    _fargs[pm.group(1)] = json.loads(_raw)
                except (json.JSONDecodeError, ValueError):
            ...
            tool_calls.append({
                ...
                "type": "function",
                "function": {"name": _fname,
                             "arguments": json.dumps(_fargs, ensure_ascii=False)},
            })
            used_ranges.append((fm.start(), fm.end()))
    ...
    if not tool_calls and "<tool_call>" in text and search_text != text:
        # think 块里藏着调用：用原文再解析一次（含 think），命中即采用
        _t2, _c2 = _parse_prompt_tool_calls(text.replace("<think>", "").replace("</think>", ""))
        if _c2:
            # 可见文本仍要剥掉 think **内容**（只去掉标签会把推理留在正文里）
            _visible = re.sub(r"<think>.*?</think>", "", text, flags=re.DOTALL)
            _visible = re.sub(r"<tool_call>.*?</tool_call>", "", _visible, flags=re.DOTALL)
            return _visible.strip(), _c2
    ...
    # 任何残留的 <tool_call> 块都不交付（09-19：有调用解析成功时，残留标记曾被原样发给用户）
    if _TOOLCALL_ANY_RE.search(clean):
        clean = _TOOLCALL_ANY_RE.sub("", clean)
    if not tool_calls:
        _salvaged = _salvage_invoke_style(text)
        if _salvaged:
            tool_calls = _salvaged
            clean = _scrub_tool_markup(clean)
    clean = _scrub_tool_markup(clean)
    for _tc in tool_calls:                      # 键名归一（name→skill_name 等）
        try:
            _a = json.loads(_tc["function"]["arguments"] or "{}")
            _tc["function"]["arguments"] = json.dumps(
                _apply_arg_aliases(_tc["function"]["name"], _a), ensure_ascii=False)

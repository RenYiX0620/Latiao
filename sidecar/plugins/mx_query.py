    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": "金融数据查询语句。已验证可用的措辞（务必照此模式，其他措辞常返回空）：个股行情/财务=\"贵州茅台 股价\"或\"贵州茅台 2024 财务数据\"；指数K线=\"上证指数 近5个交易日 日线 收盘价 成交量\"；均线=\"上证指数 5日均线 10日均线\"；板块=\"半导体板块 今日涨跌幅 资金流向\"（一次只查一个板块，多板块请分多次调用）；资金=\"上证指数 主力资金净流入 今日\"。⚠️\"全部A股(板块)涨跌排行\"类汇总查询无明细数据，不要用——板块排行请改用 tavily_search。"
                }
            },
            "required": ["query"]
        }

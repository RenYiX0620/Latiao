...
class MXData:
    ...
    def write_output_files(
        ...
        for table in tables:
            ...
            for col_idx, fieldname in enumerate(table["fieldnames"], 1):
...
def cleanup_old_outputs(output_dir: Path, max_age_days: int = 3) -> int:
    """Delete mx_output files older than max_age_days.
    Returns the number of files removed. Runs after each query so the
    directory never accumulates stale snapshots indefinitely."""
    if not output_dir.exists():
        return 0
    cutoff = time.time() - max_age_days * 86400
    removed = 0
    for f in output_dir.iterdir():
        # Only touch our own data files, skip subdirs and dotfiles
...
def main():
    ...
    if len(sys.argv) >= 3 and sys.argv[1] == "--query":
        # Support --query flag (used by mx_query plugin)
    ...
    try:
        ...
        # Clean up files older than 3 days (financial data goes stale quickly)

...
| 🧵 **多会话并行** | 多个会话同时跑，每个都有完整的上下文窗口 |
| 🧩 **技能与插件** | SKILL.md 知识包 + 可编辑 Python 插件（你的修改跨版本保留） |
| 🛒 **扩展市场** | 浏览并一键安装社区技能/插件，安装前有权限确认 |
...
| 🗣️ **语音朗读** | 本地语音合成——Kokoro（100+ 中文音色）或 IndexTTS-2.5 克隆音色，按需朗读回复 |
...
- 内置引擎随每次发布**自动取上游 llama.cpp 最新预编译包**，无需手动升级；
...
| **文档生成器** | 文件（只读） | README/API 文档/变更日志——内容交回主对话写盘（需你确认） |
| **翻译助手** | 文件（只读） | 多语言与本地化——译文交回主对话写盘（需你确认） |
